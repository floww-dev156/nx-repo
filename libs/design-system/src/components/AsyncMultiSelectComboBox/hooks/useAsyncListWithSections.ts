...
const updateSections = (
   ...
   let existingItems = cacheItems
   actionItems.forEach(section => {
      const similarSection = existingItems.find(
         item => item.section === section.section
      )

      if (similarSection) {
         similarSection.children = [
            ...new Set([...similarSection.children, ...section.children])
         ]
         setShouldInvalidate && setShouldInvalidate(true, similarSection.key)
      } else {
         existingItems.push(section)
         setShouldInvalidate && setShouldInvalidate(false)
      }
   })
         ...
         switch (action.type) {
            case 'success':
               ...
               return {
                  ...
                  state: 'idle',
                  //@ts-ignore
                  items: updateSections(
                     [...data.items],
                     //@ts-ignore
                     [...action.items],
                     data.setShouldInvalidate
                  ),
                  //@ts-ignore
...
export function useAsyncList<T, C = string>(
   ...
   useEffect(() => {
      //NOTE: The below props restricts the API call on mount.
      if (shouldTriggerAPIOnMount) dispatchFetch({ type: 'loading' }, load)
