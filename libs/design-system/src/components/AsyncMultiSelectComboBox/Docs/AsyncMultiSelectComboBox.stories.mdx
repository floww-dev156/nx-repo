...
<Meta
   title='MDX/AsyncMultiSelectComboBox'
   component={AsyncMultiSelectComboBox}
/>

export const SampleAsyncMultiSelectComboBox = args => (
   <AsyncMultiSelectComboBox {...args} />
)

## AsyncMultiSelectComboBox

This Doc will gives you a brief description of the AsyncMultiSelectComboBox component.

**AsyncMultiSelectComboBox Props**
...
interface AsyncMultiSelectComboBox
   extends SpectrumComboBoxProps,
...
-  **selectedItems** : This prop is used to render selected items. In our async use cases we may get cases where we will have the selected keys but we have not fetched that data api in combobox. In that type of cases
   we can send selectedItems by using our store data so that we can show selected items by default. And this is useful to show selected items eventhough our filter api returns an empty list as result.
...
-  Send selected items to render default selecte items. Sending selected items will render selected items data eventhough the filter api returns an empty list
